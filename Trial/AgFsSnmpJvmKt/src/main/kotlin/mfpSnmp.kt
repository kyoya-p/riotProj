        ...
        runCatching {
            scheduleFlow(query.schedule).collectLatest {
                ...
                snmp.sendFlow(pdu = PDU(GETNEXT, reqVBL).toSnmp4j(), target = target).collect {
                    ...
                    setWithShardCounter(db.collection("device/${dev.id}/logs/0/counter"), 10) {
...
@ExperimentalCoroutinesApi
suspend fun setWithShardCounter(counter: CollectionReference, dispersion: Int, op: () -> Unit) {
    @Serializable
    data class Counter(val count: Int = 0)
    counter.firestore.runTransactionSuspendable {
        val r = _rand.nextInt(dispersion)
        val counterShard = counter.document("$r")
        val c = counterShard.dataAs<Counter>() ?: Counter(0)
        op()
        println("c=$c")//TODO
        counterShard.set(Counter(c.count + 1))
    }
}

inline suspend fun <T> Firestore.runTransactionSuspendable(crossinline callback: suspend (Transaction) -> T) =
